...
if results:
    ...
    plt.subplot(1, 2, 1)
    plt.scatter(value_iteration_time_list, heuristic_time_list, label="Heuristic vs Value Iteration")
    plt.title("Execution Time (ms)")
    plt.legend()

    # Plot for number of steps
    plt.subplot(1, 2, 2)
    plt.scatter(value_iteration_steps_list, heuristic_steps_list, label="Heuristic vs Value Iteration")
